...
export function ProfileCard({ username, img, coverImg, biography, name, currentUser, premium, colorTheme, bgTheme, bgDivsTheme,glowColor, userID, params }) {
    ...
    const colorCondition = {color: bgDivsTheme == '#ffffff' && bgTheme == '#ffffff' || bgDivsTheme == '#ffffff' || bgTheme == '#ffffff'   ? '#000000' : '#ffffff'}

    const borderCondition = {
        color: bgDivsTheme == '#ffffff' && bgTheme == '#ffffff' || bgDivsTheme == '#ffffff' || bgTheme == '#ffffff'  ? '#000000' : '#ffffff',
        borderColor: bgDivsTheme == '#ffffff' && bgTheme == '#ffffff' || bgDivsTheme == '#ffffff' || bgTheme == '#ffffff'  ? '#000000' : '#ffffff'
    }
    ...
    return (
        <section className="profile-info" style={{
            ...
            linear-gradient(to bottom, ${bgDivsTheme} 58%, ${bgTheme} 92%)
            `,
            border: `1px solid ${bgDivsTheme}`,
                ...
                <div className="profile-info__user-info-img-text">
                    ...
                    <div className="profile-info__user-text">
                        <div className="profile-info__user-text__name-section">
                            <h3  style={colorCondition} className={`profile-info__name`}>{name ? name : 'John'}</h3>
                            {premium && <FontAwesomeIcon icon={faCrown} color='#5D3FD3' />}
                        </div>
                        <h4 style={colorCondition} className='profile-info__username'>{username ? `@${username}` : '@JohnDoe'}</h4>
                        <h4 style={colorCondition}>{biography ? biography : 'Hi im new in falaxart :D'}</h4>
